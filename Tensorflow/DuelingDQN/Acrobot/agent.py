...
def QModelMean(input_shape=(6,), output_shape=3):
...
def QModelMax(input_shape=(6,), output_shape=3):
    inputs = tf.keras.layers.Input(shape=input_shape)
    x = tf.keras.layers.Dense(32, activation='selu')(inputs)
    x = tf.keras.layers.Dense(32, activation='selu')(x)
...
if __name__ == '__main__':
    state = np.random.randn(NUM_ENVS,6)
    agent = Agent(input_shape=(6,),output_shape=3, num_envs=NUM_ENVS)
